### Task 1: MySQL stored procedure with create-only MD5 password

**Files:**
- Create: `docs/superpowers/plans/sql/web_upsertEmployee.sql`
- Apply manually in MySQL Workbench / CLI against `employee_attendance`

**Interfaces:**
- Consumes: existing `employee`, `employeelocation` tables; existing location CSV/`JSON_TABLE` logic
- Produces: `CALL web_upsertEmployee(p_empId, p_fullName, p_joiningDate, p_email, p_mobileNumber, p_roleId, p_designationId, p_locationIds, p_password)` returning `{ empId, message }`

- [ ] **Step 1: Write the SP SQL file**

Create `docs/superpowers/plans/sql/web_upsertEmployee.sql` with exactly:
...
- [ ] **Step 2: Apply the SP in MySQL**

Run the SQL file against the `employee_attendance` database (Workbench or):

```bash
mysql -u root -p employee_attendance < docs/superpowers/plans/sql/web_upsertEmployee.sql
```

Expected: procedure recreated with 9 parameters; no errors.

- [ ] **Step 3: Smoke-check SP signature**

```sql
SHOW CREATE PROCEDURE web_upsertEmployee\G
```

Expected: `p_password VARCHAR(100)` is the last IN parameter; INSERT uses `MD5(p_password)`.
